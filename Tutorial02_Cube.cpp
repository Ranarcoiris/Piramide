...
void Tutorial02_Cube::CreatePipelineState()
{
    ...
    PSOCreateInfo.PSODesc.Name = "Pyramid PSO";
...
void Tutorial02_Cube::CreateVertexBuffer()
{
    ...
    // Pyramid vertices
    constexpr Vertex PyramidVerts[5] =
        {
            // Base vertices
            {float3{-1, -1, -1}, float4{1, 0, 0, 1}}, // 0: Bottom-left
            {float3{+1, -1, -1}, float4{0, 1, 0, 1}}, // 1: Bottom-right
            {float3{+1, -1, +1}, float4{0, 0, 1, 1}}, // 2: Top-right
            {float3{-1, -1, +1}, float4{1, 1, 0, 1}}, // 3: Top-left

            // Apex vertex
            {float3{0, +1, 0}, float4{1, 0, 1, 1}} // 4: Apex
        };

    // Create a vertex buffer that stores pyramid vertices
...
void Tutorial02_Cube::Render()
{
    ...
    DrawAttrs.NumIndices = 18;        // 6 triangles * 3 vertices = 18 indices
...
void Tutorial02_Cube::Update(double CurrTime, double ElapsedTime)
{
    ...
    // Apply rotation
    float4x4 PyramidModelTransform = float4x4::RotationY(static_cast<float>(CurrTime) * 1.0f) * float4x4::RotationX(-PI_F * 0.1f);
    ...
    m_WorldViewProjMatrix = PyramidModelTransform * View * SrfPreTransform * Proj;
}
